...
// CHTrackController : Change Track Controller
// Değişiklik Takip Kontroller
// updated:15.11.2024
// created by: Deniz Göksel
namespace DESTAP.Controllers
{
    public class CHTrackController : Controller
    {
        ...
        public CHTrackController(DB_Context context, EmailService emailService, CommonController commonsController)
        {
            _context = context;
        ...
        {
            var records = _context.TB_ChangeTrack
        ...
        public async Task<IActionResult> Create()
        {
            ...
            var model = new ChangeTrackModel();
            return View(model);
        }
        ...
        public async Task<IActionResult> Create(ChangeTrackModel changeTrack, List<IFormFile> fileUploads)
        {
            if (ModelState.IsValid)
            {
                // Save the record to the database first
                _context.TB_ChangeTrack.Add(changeTrack);
                await _context.SaveChangesAsync();

                // Save the files and update the FilePath property
                var filePaths = await _commonsController.CreateFiles(changeTrack, fileUploads);
                changeTrack.FilePath = string.Join(",", filePaths); // Concatenate file paths
                _context.Update(changeTrack); // Update the model with file paths
                await _context.SaveChangesAsync();

                // Send email notification
                var userId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
                var maxId = await _context.TB_DVTrack.MaxAsync(u => u.ID);
                await _commonsController.SendEmail(userId, maxId, "TEST MAIL DK KAYDI OLUŞTURULDU", "CHTrack");
        ...
        {
            var record = await _context.TB_ChangeTrack.FindAsync(id);
            var departments = await _context.GetDepartmentsFromProcedureAsync();
            ViewBag.Departments = new SelectList(departments, "Department", "Department");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, ChangeTrackModel model)
        {
            if (id != model.ID)
            {
                return BadRequest();
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    await _commonsController.SendEmail(User.FindFirst(ClaimTypes.NameIdentifier).Value, id, "TEST MAİLİ DK KAYDI GÜNCELLENDİ.", "CHTrack");
        ...
        public async Task<IActionResult> DeleteConfirmed(int? id)
        {
            ...
            try
            {
                await _context.Database.ExecuteSqlRawAsync("EXEC BackupAndDeleteRecord_Change @RecordID = {0}", id);
            }
            catch (Exception e)
            {
        ...
        public async Task<JsonResult> SendFeedback(int UserID, int RecordID, string FeedbackDescription)
        {
            ...
            {
                // Kayıt verisini al
                var record = await _context.TB_ChangeTrack
                ...
                var subject = "TEST MAIL - DK Yeni Geri Bildirim";
                var body = $"Geri Bildirim: {FeedbackDescription}";

                var mailModel = await _commonsController.CreateMailModel(User.FindFirst(ClaimTypes.NameIdentifier).Value, RecordID, "KALİTE GÜVENCE", subject, body, null, "CHTrack");
        ...
        public async Task<JsonResult> CompleteRecord(int RecordID, DateTime CompletionDate, string CompletionDescription)
        {
            try
            {
                ...
                // Mail içeriğini oluşturmak için CreateMailModel metodunu kullan
                var subject = "TEST-MAIL DK Tamamlanmış Kayıt Bildirimi";
        ...
        public async Task<IActionResult> SubmitTargetDateChange(int recordId, string requestedDate, string changeDescription)
        {
            try
            {
                ...
                var emailSubject = "TEST Mail - DK Hedef Tarihi Değişikliği Talebi";
                ...
                var mailModel = await _commonsController.CreateMailModel(User.FindFirst(ClaimTypes.NameIdentifier).Value, recordId, "KALİTE GÜVENCE", emailSubject, emailBody, null, "CHTrack");
