...
// CPTrackController : Corrective and Preventive action
// Düzeltici ve Önleyici faaliyet
// updated:15.11.2024
// created by: Deniz Göksel
namespace DESTAP.Controllers
{
    public class CPTrackController : Controller
    {
        ...
        public CPTrackController(DB_Context context, EmailService emailService, CommonController commonsController)
        {
            _context = context;
        ...
        {
            var records = _context.TB_CPATrack
                                  .Include(u => u.ResponsibleUser)
                                  .ToList();  // Tüm kayıtları alToList();  // Tüm kayıtları al
        ...
        {
            var record = await _context.TB_CPATrack.FindAsync(id);
            var departments = await _context.GetDepartmentsFromProcedureAsync();
            ViewBag.Departments = new SelectList(departments, "Department", "Department");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, CPATrackModel model)
        {
            if (id != model.ID)
            {
                return BadRequest();
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    await _context.SaveChangesAsync();
                    await _commonsController.SendEmail(User.FindFirst(ClaimTypes.NameIdentifier).Value, id, "TEST MAİLİ DÖF KAYDI GÜNCELLENDİ.", "CPATrack");
                    return RedirectToAction(nameof(Index));
                }
                catch (DbUpdateException ex)
                {
                    // Veritabanı güncelleme hatası durumunda detaylı hata mesajı
                    var errorMessage = ex.InnerException?.Message ?? ex.Message;
                    ModelState.AddModelError("", "Veri güncellenirken bir hata oluştu: " + errorMessage);
                }
                catch (Exception ex)
                {
                    // Genel hata durumu
        ...
        public async Task<IActionResult> DeleteConfirmed(int? id)
        {
            ...
            try
            {
                await _context.Database.ExecuteSqlRawAsync("EXEC BackupAndDeleteRecord_CPA @RecordID = {0}", id);
            }
            catch (Exception e)
            {
        ...
        public async Task<JsonResult> SendFeedback(int UserID, int RecordID, string FeedbackDescription)
        {
            ...
            {
                // Kayıt verisini al
                var record = await _context.TB_CPATrack
                ...
                var subject = "TEST MAIL - DÖF Yeni Geri Bildirim";
                var body = $"Geri Bildirim: {FeedbackDescription}";
                //
                var mailModel = await _commonsController.CreateMailModel(User.FindFirst(ClaimTypes.NameIdentifier).Value, RecordID, "KALİTE GÜVENCE", subject, body, null, "CPATrack");
        ...
        public async Task<JsonResult> CompleteRecord(int RecordID, DateTime CompletionDate, string CompletionDescription)
        {
            try
            {
                ...
                // Mail içeriğini oluşturmak için CreateMailModel metodunu kullan
                var subject = "TEST-MAIL DÖF Tamamlanmış Kayıt Bildirimi";
        ...
        public async Task<IActionResult> SubmitTargetDateChange(int recordId, string requestedDate, string changeDescription)
        {
            try
            {
                ...
                var emailSubject = "TEST Mail - DÖF Hedef Tarihi Değişikliği Talebi";
                ...
                var mailModel = await _commonsController.CreateMailModel(User.FindFirst(ClaimTypes.NameIdentifier).Value, recordId, "KALİTE GÜVENCE", emailSubject, emailBody, null, "CPATrack");
